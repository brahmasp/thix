...
class FromGymnasium(embodied.Env, Generic[U, V]):
  ...
  def obs_space(self):
    ...
    for k, v in spaces.items():
      if isinstance(v, gymnasium.spaces.Dict):
        flat_v = self._flatten(v)
        conv_spaces.update({f'{k}/{sub_k}': self._convert(sub_v) for sub_k, sub_v in flat_v.items()})
      else:
        conv_spaces[k] =  self._convert(v)
