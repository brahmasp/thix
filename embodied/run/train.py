...
def train(make_agent, make_replay, make_env, make_logger, args):
  ...
  def log_step(tran, worker):
    ...
    if worker < args.log_video_streams:
      ...
      for key in args.log_keys_unroll_video:
        if key in tran:
          episode.add(f'policy_{key}', tran[key], agg='concat')
    for key, value in tran.items():
      if re.match(args.log_keys_sum, key):
        episode.add(f'sum_{key}', value, agg='sum') # TODO if this works do for all run scripts
      if re.match(args.log_keys_avg, key):
        episode.add(f'mean_{key}', value, agg='avg')
      if re.match(args.log_keys_max, key):
        episode.add(f'max_{key}', value, agg='max')
