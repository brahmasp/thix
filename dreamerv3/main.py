...
parser.add_argument('--wandb.run_name', type = str)
...
def make_agent(config):
  from . import agent as agt
  from . import thick_agent as thick_agt
  env = make_env(config, 0)
  if config.agent == 'random':
    agent = embodied.RandomAgent(env.obs_space, env.act_space)
  elif config.agent == 'base':
    agent = agt.Agent(env.obs_space, env.act_space, config)
  elif config.agent == 'thick':
    agent = thick_agt.ThickAgent(env.obs_space, env.act_space, config)
  else:
      raise NotImplementedError(f"Unknown agent: {config.agent}")
  env.close()
  return agent
...
def make_env(config, index, **overrides):
  ...
  ctor = {
      ...
      'kitchen': 'embodied.envs.franka_kitchen:FrankaKitchen',
      'pokemon': 'embodied.envs.pokegym:Pokemon',
  }[suite]
...
def print_banner(config):
    if config.agent == 'base':
        ...
        embodied.print(r" ___________  ___ ___   ___   _______   ____  __  ")
        embodied.print(r" \__    ___/ /   |   \ |   | /   ___ \ |    |/ _| ")
        embodied.print(r"   |    |   /    ~    \|   |/    \  \/ |      <   ")
        embodied.print(r"   |    |   \    Y    /|   |\     \____|    |  \  ")
        embodied.print(r"   |____|    \___|___/ |___| \________/|____|___\ ")
