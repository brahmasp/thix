...
def expand_repeat(a, axis, repeats):
  b = jnp.expand_dims(a, axis)
  return jnp.repeat(b, repeats, axis)
...
def broadcast_to_match(a, b):
  assert a.shape == b.shape[:len(a.shape)]

  new_shape = a.shape + (1,) * (b.ndim - a.ndim)

  a_reshaped = jnp.reshape(a, new_shape)
  a_broadcasted = jnp.broadcast_to(a_reshaped, b.shape)

  return a_broadcasted

def batched_cosine_similarity(x, y):
  x_norm = jnp.linalg.norm(x, axis=-1, keepdims=True)
  y_norm = jnp.linalg.norm(y, axis=-1, keepdims=True)
  return jnp.einsum('...k,...k->...', x/x_norm, y/y_norm)